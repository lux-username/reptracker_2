...
## 2026-07-16 — Ship #43 without counsel review: accepted risk, not a standing gate

**Recorded that the "counsel read before launch" caveat on #43 is not a gate — it is an accepted residual risk.** There is no counsel and there won't be: this is a free, no-revenue tool built by a solo hobbyist. A gate that can never be satisfied isn't a gate; it just reads as "never launch," which is not the intent. The owner explicitly accepts shipping the privacy page without legal sign-off.

**Basis for acceptance:** the compliance work (#26) found the project substantially compliant; the CalOPPA page (#43) carries every required clause; and its load-bearing FTC §5 claim was verified literally true end-to-end (see the #43 decision above). This is a genuine good-faith effort at the standard a hobby project can meet — it establishes the posture, and the owner accepts the residual legal risk of not having a lawyer confirm the wording.

**Consequence:** removed the "standing pre-launch gate" framing and the two blocker bullets from STATUS.md. The infra-invariant that the privacy claim depends on (stay on Hobby / no body-capturing log drain / no payload-capturing observability integration) remains recorded in the #43 decision above as a *forever* "don't-regress" constraint — not a pre-launch task, and no open issue, since there is no pending action. #45 (contingent abuse escalation) is the only open issue.

## 2026-07-23 — `CLAUDE.md`'s Stack section replaced with a pointer, not corrected in place

**Chose to delete the duplicated stack description and point at `spec.md` §Tech stack, over correcting the copy and keeping `CLAUDE.md` self-contained.** The section had accumulated four independent stale claims: an Anthropic/Haiku LLM layer retired in session 6, "one page, one route" (now two pages + two route handlers), cheerio "in a route handler" (it is in `lib/`), and Vercel Cron as the scraper's scheduler (the real cadence is GitHub Actions; Vercel Hobby is daily-only). Every one of them was a *copy* that drifted from an original in `spec.md`. Correcting the copy would have fixed four facts and left the mechanism that produced them, so the copy was removed instead — this is the "every fact has exactly one home" rule applied to the file that states the rule.

**How the drift was caught:** not by audit, but by consequence. I recommended a "Brief"-family product name on the grounds that "brief implies digest, which is literally what the LLM produces" — reasoning from `CLAUDE.md:12` instead of from the code, where `lib/summaries.ts:1` says "NO LLM" and `package.json` has no `anthropic` dependency. The owner challenged it. A stale doc did not merely sit there being wrong; it propagated into a naming recommendation that would have put a false promise on the masthead.

**Retained in `CLAUDE.md`, deliberately:** two constraints that are *working practice* rather than stack description — that there is no LLM **by decision** (with a "do not reintroduce generated prose without revisiting this" instruction), and that the scraper cadence is split across two schedulers (change one, check the other, including `CRON_SECRET` in both places). A bare pointer would have lost both, and the second is the kind of fact whose absence causes a broken deploy.

**Known cost, accepted:** `spec.md` §Tech stack currently carries two of the same errors, so until #47 lands the pointer sends readers to stale facts — arguably worse than the duplication, because it launders them as authoritative. Accepted on the basis that #47 is a small, filed, prioritized fix and the alternative preserves the drift mechanism permanently.

## 2026-07-23 — Two constraints any product name must clear (naming candidate → #46)

**Recorded independently of which name wins**, because both derive from decisions already made and will outlive the current candidate:

1. **No synthesis.** The LLM was retired in session 6 — bill text is CRS summaries rendered verbatim, and the product's neutrality is a property of *what is rendered*, not of a prompt. Any name promising an editorial condensation is therefore a false promise: **Brief**, **Digest**, **Report**, **Summary**. ("Brief" fails twice over — in a legislative context it also reads as *legal brief*, and the noun sense in its etymology is literally a papal letter, then a legal writ.)
2. **No decision-overclaim.** #30 retired "upcoming decisions" as an overclaim: a hearing is not a decision, and the sponsored-bills half is not one either. This rules out **On the Docket**, **Before the Vote**, **Coming to a Vote**. Note **docket** fails *both* tests — it currently denotes a list of matters awaiting decision, and etymologically it meant a summary or abstract.

**Also established, as reusable naming reasoning:** "tracker" is rejected on stance (it frames the representative as a subject under watch, inverting the constituent-as-principal relationship) and secondarily on crowding (GovTrack has held the word since 2004, alongside Countable and POPVOX). "Activity" is preferred as the House's own noun — its floor-schedule section is literally `house.gov/legislative-activity` — but is the one word in the candidate that could be misread as a person's *whereabouts*, which is the exact failure mode being avoided; it therefore requires either a bounding adjective or a subhead adjacent to it. Between the two three-word forms, "Representative Activity Lookup" was preferred over "Legislative Activity Lookup" on **recoverability**: "legislative" is recoverable from "representative" (a representative's activity *qua* representative is legislative), while "representative" is not recoverable from "legislative" (whose activity?).

**The name itself is not decided** — it is filed as **#46** for the owner, with scope, objections, and the rejected alternatives recorded there. This entry exists so that a future rename debate starts from the constraints rather than re-deriving them.
